 ...
 * Description: TikTok-style Reels with vertical scroll and profile grid playback.
 * Version: 2.5
 * Author: Your Name
 */
...
add_shortcode('bpr_upload_form', function(){
    if (!is_user_logged_in()) return '<p>Login to upload.</p>';
    ob_start();
    if (isset($_GET['bpr_msg'])) {
        $m = $_GET['bpr_msg'];
        $text = ['success'=>'Upload OK','error_post'=>'Error saving','error_upload'=>'Upload failed','error_nofile'=>'Select video'][$m] ?? '';
        $cls = strpos($m,'error')===0 ? 'error' : 'success';
        echo '<div class="bpr-message bpr-'.$cls.'">'.esc_html($text).'</div>';
    }
...
add_shortcode('bpr_reels_feed', function(){
    $opts = get_option('bpr_settings');
    $q = new WP_Query(['post_type'=>'bpr_reel','post_status'=>'publish','posts_per_page'=>-1,'orderby'=>'date','order'=>'DESC']);
    ob_start(); ?>
    <div class="bpr-feed">
      <?php while($q->have_posts()): $q->the_post();
        $vid = wp_get_attachment_url(get_post_meta(get_the_ID(),'bpr_video',true));
        $aid = get_the_author_meta('ID'); $avatar = function_exists('bp_core_fetch_avatar') ? bp_core_fetch_avatar(['item_id'=>$aid,'html'=>false,'width'=>48,'height'=>48]) : get_avatar_url($aid,['size'=>48]);
        ...
        <div class="bpr-video-wrapper">
            ...
            <div class="bpr-mute-toggle" title="Toggle sound">🔇</div>
            <div class="bpr-overlay">
                <a href="<?php echo esc_url($purl); ?>" class="bpr-author-avatar-link"><div class="bpr-author-avatar" style="background-image:url('<?php echo esc_url($avatar); ?>')"></div></a>
                <a href="<?php echo esc_url($purl); ?>" class="bpr-author-name-link"><span class="bpr-author-name"><?php echo esc_html(get_the_author_meta('display_name',$aid)); ?></span></a>
                <div class="bpr-author-texts"><h3><?php the_title(); ?></h3><p><?php the_content(); ?></p></div>
            </div>
...
add_shortcode('bpr_reels_grid', function(){
    ...
    $q = new WP_Query(['post_type'=>'bpr_reel','author'=>$uid,'post_status'=>'publish','posts_per_page'=>-1,'orderby'=>'date','order'=>'DESC']);
    ?>
    <div class="bpr-grid-wrapper">
      <?php while($q->have_posts()): $q->the_post();
        $vid = wp_get_attachment_url(get_post_meta(get_the_ID(),'bpr_video',true)); if (!$vid) continue; ?>
