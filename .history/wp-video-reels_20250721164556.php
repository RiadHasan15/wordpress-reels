 ...
 * Description: Allow users to upload and view vertical video reels (like Instagram or TikTok).
 * Version: 1.1
 * Author: Your Name
 */

// Register 'reel' post type
...
function wpvr_upload_form_shortcode() {
    if (!is_user_logged_in()) {
        return '<p>Please <a href="/wp-login.php">log in</a> to upload a reel.</p>';
    }
...
function wpvr_feed_shortcode() {
    ...
    while ($query->have_posts()) {
        ...
        if ($video_url) {
            echo '<div class="wpvr-reel">
                    <div class="wpvr-video-frame">
                        <video class="wpvr-video" src="' . esc_url($video_url) . '" autoplay muted loop playsinline></video>
                    </div>
