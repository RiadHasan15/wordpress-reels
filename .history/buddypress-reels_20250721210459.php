 ...
 * Description: BuddyPress integrated video reels with vertical scroll feed and TikTok-style grid on profiles.
 * Version: 2.5
 */
...
add_action('wp_enqueue_scripts', function() {
    ...
    wp_localize_script('bpr-scripts', 'bprSettings', $opts);
});

// Handle reel upload form submission
...
function bpr_handle_upload() {
    if (!is_user_logged_in()) wp_die('Please login to upload reels.');
...
add_shortcode('bpr_upload_form', function() {
    if (!is_user_logged_in()) return '<p>Please login to upload reels.</p>';
    ob_start();
    if (isset($_GET['bpr_msg'])) {
        $msg = $_GET['bpr_msg'];
        $map = [
            'success'      => 'Upload successful!',
            'error_post'   => 'Error creating reel post.',
            'error_upload' => 'Error uploading video.',
            'error_nofile' => 'No video selected.',
        ];
        $cls = strpos($msg, 'error') === 0 ? 'error' : 'success';
        echo '<div class="bpr-message bpr-' . esc_attr($cls) . '">' . esc_html($map[$msg] ?? '') . '</div>';
    }
    include plugin_dir_path(__FILE__) . 'templates/upload-form.php';
    return ob_get_clean();
});

// Vertical reels feed shortcode
        ...
        <?php while ($q->have_posts()): $q->the_post();
            ...
            <div class="bpr-video-wrapper">
                ...
                <div class="bpr-overlay">
                    <a href="<?php echo esc_url($purl); ?>" class="bpr-author-avatar-link" title="View profile">
                        <div class="bpr-author-avatar" style="background-image:url('<?php echo esc_url($avatar); ?>')"></div>
                    </a>
                    <a href="<?php echo esc_url($purl); ?>" class="bpr-author-name-link" title="View profile">
...
// Profile grid shortcode (3 columns) with modal player
add_shortcode('bpr_reels_grid', function() {
    ...
    <?php include plugin_dir_path(__FILE__) . 'templates/profile-grid-modal.php'; ?>
    <?php
    return ob_get_clean();
});

// Add BuddyPress Reels tab to user profile
