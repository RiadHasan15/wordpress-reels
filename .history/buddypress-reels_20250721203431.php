 ...
 * Description: Vertical video reels with frontend upload and BuddyPress integration with profile grid view.
 * Version: 2.4
 * Author: Your Name
 */
...
add_shortcode('bpr_reels_feed', function($atts) {
    ...
    $atts = shortcode_atts([
        'author' => 0,
        'grid' => 0, // new param: 1 = grid mode, 0 = vertical reels
    ], $atts);
    ...
    if ($atts['grid'] == 1) {
        ...
        while ($query->have_posts()) : $query->the_post();
            ...
            // Thumbnail is first frame or video poster, fallback to video itself for simplicity
            ?>
            <div class="bpr-grid-item">
                <video
                    class="bpr-grid-video"
                    muted
                    loop
                    preload="metadata"
                    playsinline
                ...
                <div class="bpr-grid-title"><?php the_title(); ?></div>
            </div>
        <?php endwhile; wp_reset_postdata(); ?>
        </div>
        <?php
    } else {
        // VERTICAL SCROLL REELS VIEW (existing)
        ?>
        <div class="bpr-feed">
            <?php
            while ($query->have_posts()) : $query->the_post();
...
function bpr_bp_reels_screen_content() {
    ...
    // Display reels in grid mode on profile page
    echo do_shortcode('[bpr_reels_feed author="' . intval($displayed_user_id) . '" grid="1"]');
}

// Admin settings page - unchanged
